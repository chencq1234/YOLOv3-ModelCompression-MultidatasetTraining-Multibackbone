...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default', quantized=-1, qlayers=-1):
        ...
        self.module_list, self.routs = create_modules(self.module_defs, img_size, arc, quantized, qlayers)
...
def load_darknet_weights(self, weights, cutoff=-1, train=False, pt=False):
...
def save_as_onnx(model, save_name):
    model.train(False)
    # if not self.multi_gpu:
    from torch.autograd import Variable
    # torch.save(self.model, os.path.join(self.cfg["EXP_DIR"], "model.pth"))
    dummy_input = Variable(torch.randn(1, 3, 416, 416))# c, w, h
    # Export the model
    # save_onnx_path = os.path.join(outpath, "hrnet32.onnx")
    # save_onnx_path = os.path.realpath(__file__).replace('.py', '.onnx')
    # save_onnx_path = os.path.join(os.path.curdir, save_name)
    save_onnx_path = save_name
...
if __name__ == '__main__':
    ...
    # net = Darknet('cfg/yolov3-mobilenet/yolov3-mobilenet-car.cfg', arc='default')
